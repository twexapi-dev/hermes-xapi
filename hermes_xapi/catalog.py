...
def _load_raw() -> list[dict[str, Any]]:
    text = (
        files(__package__ or "hermes_xapi")
...
def _endpoint(item: dict[str, Any]) -> Endpoint:
    return Endpoint(
        ...
        cost=str(item["cost"]) if item.get("cost") else None,
...
def explore(args: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    risk = _optional_text(args.get("risk")).lower()
    if risk:
        endpoints = tuple(endpoint for endpoint in endpoints if endpoint.risk.lower() == risk)
